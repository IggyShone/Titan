...
titanApp.controller('QueueController', ['$scope', '$filter', '$location','$timeout','$modal', function ($scope, $filter, $location, $timeout, $modal) {
    ...
    var
        ...
        //createdtimeList = [$filter('date')(randomDate(new Date(2015, 4, 13, 10, 10, 10), new Date(2015, 4, 13, 10, 10, 10)), 'yyyy-MM-dd HH:mm:ss ')],
    ...
    $scope.draggable = 'none';


    $scope.markRowDraggable = function(event){
        $(event.currentTarget).children("td:first-child").children("i").css("visibility", "visible");
        $scope.draggable = 'move';
    }

    $scope.unMarkRowDraggable = function(event){
        $scope.draggable = 'none';
        $(event.currentTarget).children("td:first-child").children("i").css("visibility", "hidden");
    }
    ...
    $scope.dates = { startDate: '05 16 2015', endDate: '05 16 2015' };
    ...
    $scope.sortableOptions = {
        stop: function(e, ui) {
            for (var index in $scope.todos) {
                $scope.rowCollection[index].i = index;
            }
        }
    };
    ...
    $scope.up = function (row) {
        ...
        for (var i = 0; i < $scope.rowCollection.length; i++) {
            var row = $scope.rowCollection[i];
            if(row.isSelected == true) {
                if(i<$scope.rowCollection.length){
                    last = $scope.rowCollection[$scope.rowCollection.length-1];
                    temp = last;
                    $scope.rowCollection[$scope.rowCollection.length-1] = row;
                    $scope.rowCollection[i] = temp;
                }
            }
        }
    }
    ...
    $scope.down = function (row) {
        ...
        for (var i = 0; i < $scope.rowCollection.length; i++) {
            ...
            if(row.isSelected == true) {
                if(i>=1){
                    firstRow = $scope.rowCollection[0];
                    temp = firstRow;
                    $scope.rowCollection[0] = row;
                    $scope.rowCollection[i] = temp;
                }
            }
        }
    ...
    $scope.dequeue = function(size){
        var modalInstance = $modal.open({
            templateUrl: 'queueContent.html',
            controller: 'ModalInstanceCtrl'
        });
    };
